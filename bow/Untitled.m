...
mkdir(fullfile(conf.calDir,'1'));
mkdir(fullfile(conf.calDir,'2'));

for k = 1:length(trueImages)
    imwrite(trueImages{k},fullfile(conf.calDir,'1', num2str(k)));
end
for k = 1:length(falseImages)
    imwrite(falseImages{k},fullfile(conf.calDir,'2',num2str(k)));
end
