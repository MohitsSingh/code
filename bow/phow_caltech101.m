function model = phow_caltech101(trueImages,falseImages)
...
conf.calDir = 'data/imgs';
conf.dataDir = 'data/' ;
p1 = fullfile(conf.calDir,'1');
p2 = fullfile(conf.calDir,'2');
if(exist(p1,'dir'))
...
for k = 1:length(trueImages)
    imwrite(trueImages{k},fullfile(p1, [num2str(k) '.jpg']),'Quality',100);
end

conf.numTrain = length(trueImages);

for k = 1:length(falseImages)
    imwrite(falseImages{k},fullfile(p2, [num2str(k) '.jpg']),'Quality',100);
end
