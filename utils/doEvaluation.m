function [recalls,precisions,F_scores,per_class_losses,all_cms]  = doEvaluation( opts,imdb,startEpoch,endEpoch,perfSuffix,setToTest )
...
N = 10;
all_cms = {};
aucs = zeros(N,endEpoch);
F_scores = zeros(N,endEpoch);
recalls = zeros(N,endEpoch);
precisions = zeros(N,endEpoch);
class_accuracies = zeros(N,endEpoch);
per_class_losses = zeros(N,endEpoch);
%%
for t = startEpoch:endEpoch
    ...
    cm = confMatrix( curBatchLabels, IDXpred, N );
    
    curBatchResults = reshape(curBatchResults,1,1,N,[]);
    loss_opts.loss = 'log';
