...
% % % clean out false results from dirs...
% % sel_to_clean = ~[s40_fra.isTrain] & [s40_fra.indInFraDB]~=-1;
% % imgsToClean = {s40_fra(sel_to_clean).imageID};
% % outDirs = {'~/storage/fra_landmarks','~/storage/s40_fra_face_seg',...
% %     '~/storage/s40_my_facial_landmarks',...
% %     '~/storage/s40_obj_prediction',...
% %     '~/storage/s40_fra_feature_pipeline_all'};    
% % 
% % for d = 1:length(outDirs)
% %     for t = 1:length(imgsToClean)
% %         [d t]
% %         delete(j2m(outDirs{d},imgsToClean{t}))
% %     end
% % end
