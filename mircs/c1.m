...
%     if (curImg.classID ~= conf.class_enum.LOOKING_THROUGH_A_MICROSCOPE)
%         continue
%     end
    if (~curImg.isTrain)
       continue;
    ...
    %I = im2uint8(I);
    all_rects = {};
    for u = 1:length(s40_face_dets(t).detections)
        curBoxes = s40_face_dets(t).detections(u).boxes;
        if isempty(curBoxes),curBoxes = -inf(1,6);end
        curBoxes(:,end+1) = s40_face_dets(t).detections(u).rot;
    ...
    [ovps,ints] = boxesOverlap(rects_axis_algnd,I_rect);
    [~,~,s] = BoxSize(rects_axis_algnd);
    top = intersect(top,find(ints./s > 0.8),'stable');
    
    s40_person_face_dets(t).imageID = curImg.imageID;
    s40_person_face_dets(t).boxes_rot = all_rects(top,:);
    s40_person_face_dets(t).boxes_algn = rects_axis_algnd(top,:);
    %     continue;
    % %
