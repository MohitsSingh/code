...
% params.roiParams.infScale = 2.5;
% params.roiParams.absScale = 192*params.roiParams.infScale/1.5;
% % load facial landmarks, images (cropped around face);
% fra_db = fra_db([fra_db.classID]==4);
% fra_db = prepareData(conf,fra_db,params);
%
% for u = 1:length(fra_db)
%     fra_db(u).I = im2uint8(fra_db(u).I);
% end
% % save ~/storage/misc/fra_db.mat fra_db

% A = load('/home/amirro/storage/data/Stanford40/annotations/faces_oct_31_14/drinking_002.mat');
%% TRAINING
...
for it = 1:length(cur_set)
    ...
    I = getImage(conf,imgData);
    gt_graph = get_gt_graph(imgData,nodes,params,I);
    [boxes,boxGraph] = getBoxGraphForImage(imgData,I,gt_graph,regionSampler);
...
%     if (imgData.classID~=1),continue,end
    imgData.imageID
    I = getImage(conf,imgData);
    configs = sampleGraph(imgData,false,nodes,edges,params,I,regionSampler,15);                
    if toVisualize
        for u = 1:length(configs)
            clf; imagesc2(I);
            p = configs{u};
            polys = cellfun2(@(x) x.poly,p);
            plotPolygons(polys,'g-');
    ...
    edgeBoxes = regionSampler.sampleEdgeBoxes(I);
    face_in_boxes = BoxIntersection2(edgeBoxes, imgData.faceBox);
    [~,~,faceArea] = BoxSize(imgData.faceBox);
    size(edgeBoxes,1)
    edgeBoxes(face_in_boxes./faceArea > .4,:) = [];
    size(edgeBoxes,1)
    [~,~,bArea] = BoxSize(edgeBoxes);   
    edgeBoxes(bArea./ prod(size2(I)) > .3,:) = [];
    
%     size(edgeBoxes,1)
%         
%     showSortedBoxes(I,edgeBoxes,-edgeBoxes(:,5),.01)
    sal_opts.show = false;
    maxImageSize = 200;
    ...
    box_sals = sum_boxes(sal1,edgeBoxes);
    [~,~,bArea] = BoxSize(edgeBoxes);   
    box_sals = box_sals./bArea;
    %     showSortedBoxes(I,edgeBoxes,bArea,.05)
%     showSortedBoxes(I,edgeBoxes,-box_sals,.1,20)
%     edgeBoxes(box_sals<sal_thresh,:) = [];
    size(edgeBoxes,1)
        
    boxGraph = buildBoxGraph(edgeBoxes,3,I);
    
%     showSortedBoxes(I,edgeBoxes,edgeBoxes(:,5),.1,10)
    %     continue
%     profile off
% % % 
...
% % %     m = {};
% % %     masks_compact = struct('perim',{},'pt',{},'box',{});
% % %     tic
% % %     for t = 1:length(masks)
% % %         t
% % %         curMask = masks(:,:,t);
% % %         curPerim = bwperim(curMask);
% % %         masks_compact(t).box = region2Box(curPerim);
% % %         perimPixels = find(curPerim);
% % %         masks_compact(t).perim = uint16(perimPixels);
% % %         [yy,xx] = find(~curPerim & curMask,1,'first');
% % %         masks_compact(t).pt = [xx yy];
% % %     end
% % %     face_in_boxes = BoxIntersection2(cand_boxes, imgData.faceBox);
% % %     % get superpixel graph...
% % %     
% % %     
% % %     cand_boxes = candidates.bboxes;
% % %     face_in_boxes = BoxIntersection2(cand_boxes, imgData.faceBox);
% % %     
% % %     showSortedBoxes(I,candidates.bboxes,candidates.scores,.01)
% % %     
    
%     for t = 1:length(masks)
%         t
%         masks_compact(t).box = uint16(masks_compact(t).box);
%         masks_compact(t).pt = uint16(masks_compact(t).pt);
%     end
%     toc
    
%     save masks_compact.mat masks_compact
% % %     
% % %     
% % %     for z = 1:length(m)
% % %         clf; 
...
% % %         % fill the polygon
% % %         
% % %         [yy,xx] = find(~v & curMask,1,'first');
% % %         plot(xx,yy,'r+')        
% % %         bw1 = bwfill(v,xx,yy,8);        
% % %         subplot(2,2,3);
% % %         imagesc2(bw1)
% % %         assert(all(bw1(:)==curMask(:)))
% % %         dpc(.01)
% % %     end

    tic
    curTestFeats = configurationToFeats(I,configs,nodes,featureExtractor);
    toc
%     profile viewer
    curTestFeats = cat(2,curTestFeats{:});
    
    curScores = w'*curTestFeats;
    %     close all
    [z,iz] = sort(curScores,'descend');
    for u = [1:5]%:length(configs)
        u
        clf; imagesc2(I);
        p = configs{iz(u)};
        polys = cellfun2(@(x) x.poly,p);
        %plotPolygons(polys,'g-');
        plotPolygons(polys(1),'r-','LineWidth',2);
        plotPolygons(polys(2),'g-','LineWidth',2);
        plotPolygons(polys(3),'m-','LineWidth',2);
        title(num2str(z(u)));
        dpc
    end
    %test_{end+1} =
