function clustering_new(conf,discovery_sets,natural_set)
% initial clustering.
...
save_sampes = 1;
ovp = .1;
[pos_samples,pos_locs] = samplePatches(conf,ids_true_train,ovp);

% clustering...
[clusters] = kMeansClustering(conf,pos_samples,pos_locs,true,conf.suffix,false);
...
%     clustersPath = fullfile(conf.cachedir,['kmeans_clusters_vis' suffix '.mat']);
%     if (~exist(clustersPath,'file'))
        [clusters] = visualizeClusters(conf,ids_true_train,clusters,64);
%         save(clustersPath,'clusters');
%     else
%         load(clustersPath);
%     end
