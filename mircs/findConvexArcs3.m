function candidates = findConvexArcs3(seglist,E,edgelist)
%
...
%
s = seglist2segs(seglist);
seglist = cell(size(s,1),1);
for k = 1:size(s,1)
    seglist{k} = [s(k,1:2);s(k,3:4)];
end

startPoints = cellfun(@(x) x(1,:),seglist,'UniformOutput',false);
endPoints = cellfun(@(x) x(end,:),seglist,'UniformOutput',false);
...
groups = enumerateGroups(G,4);
% cat(2,edgelist(groups{2}){:})
edgelist_expanded = {};
edgelist = col(seglist);
for iGroup = 1:4%length(groups) %
