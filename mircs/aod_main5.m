...
%% stage -2 training: train point-of-interaction detector
cur_set = f_train;
dlib_landmark_split;
all_interaction_rois = {};
...
%     if ~isClass(t),continue,end
    ...
    if isClass(t)
        ...
        % find where this enters the mouth        
        [pos_roi,fit_index] = fitRectangle(roiMask);
        rois = {poly2mask2(pos_roi,size2(I_sub))};
        labels = 1;
    else       
        M = zeros(size2(I_sub));
        M(1:5:end,1:5:end) = true;
        [y,x] = find(mouthMask);
        p0 = [x y];        
        [y,x] = find(addBorder(zeros(size2(I_sub)),1,1) & M);
        p1 = [x y];        
        [ii,jj] = meshgrid(1:size(p0,1),1:size(p1,1));
        nSamples = 10;
        u = vl_colsubset(1:numel(ii),nSamples,'Random');        
        rois = {};
        for iu = 1:length(u)
            rois{iu} = directionalROI_rect_lite(p0(ii(u(iu)),:),p1(jj(u(iu)),:),size(I_sub,1)/8);
%             clf; imagesc2(I_sub);
%             plotPolygons(rois{iu});
%             dpc;
        end                
        rois = cellfun2(@(x) poly2mask2(x,size2(I_sub)), rois);
        labels = -1*ones(size(rois));                                
    end
    
    all_rois{end+1} = cellfun2(@(x) maskedPatch(I_sub,x,true,.5),rois);
    all_labels{end+1} = labels;
end
...
% train generic region-of-interaction detector
% 


[pos_samples,sample_stats] = generateSamples(conf,fra_db(f_train_pos),true,nodes,params);
...
for it = 1:length(cur_set)
    ...
    thetas = 90-(0:10:180);
    rr = [cosd(thetas(:)) sind(thetas(:))];        
    rr = hingedSample(size2(I_sub)/2,.16*size(I_sub,1),size(I_sub,1)/2,thetas);
    masks = cellfun2(@(x) poly2mask2(x.xy,size2(I_sub)),rr);
%     rr = hingedSample(size2(I_sub)/2,.16*size(I_sub,1),size(I_sub,1)/4,thetas);
%     masks = [masks,cellfun2(@(x) poly2mask2(x.xy,size2(I_sub)),rr)];
%     
%     masks = cellfun2(@(x) imdilate(x,ones(5)),masks);
%     
%     Y = random(sample_stats,nToSample);
%     masks = {};
%     rois = {};
%     for t = 1:nToSample       
%         rois{t} = directionalROI_rect_lite(Y(t,:)*size(I_sub,1));
%         m = poly2mask2(rois{t},size2(I_sub));
%         if nnz(m) < 20
%             continue
%         end
%         masks{end+1} = m;
%     end    
    patches = cellfun2(@(x) maskedPatch(I_sub,x,true,.5),masks);
    curFeats = featureExtractor.extractFeaturesMulti(patches);
    curScores = w'*curFeats;
    S = computeHeatMap_regions(I_sub,masks,normalise(curScores(:)),'max');
    RR = sc(cat(3,S,im2double(I_sub)),'prob');
    clf
    subplot(1,2,1); imagesc2(RR);
    title(num2str(max(curScores)));
    subplot(1,2,2); imagesc2(I_sub);
    dpc
end
...
feats =  featureExtractor.extractFeaturesMulti(cat(2,rois));

p = Pegasos(feats,labels(:),'lambda',.1,'foldNum',5,'bias',1);
w = p.w(1:end-1);
b = p.w(end);
...
for it = 1:length(cur_set)
    ...
    mouthMask = getMouthMask(I_sub,mouthBox,imgData.Landmarks_dlib,dlib);    
    M = zeros(size2(I_sub));
%     M(1:10:end,1:10:end) = true;
%     [y,x] = find(mouthMask);
    mBox = region2Box(mouthMask);
    [x_box,y_box] = meshgrid(linspace(mBox(1)+1,mBox(3)-1,3),linspace(mBox(2)+1,mBox(4)-1,3));
    p0 = [x_box(:) y_box(:)];
    p0(~inMask(mouthMask,p0),:) = [];
%     p0(~mouthMask(sub2ind2(size(mouthMask),round(fliplr(p0))
%     x2(I_sub); plot(x_box(:),y_box(:),'r.');
%     p0 = [x y];    
%     p0 = mean(box2Pts();
    rois =  {};
    h = size(I_sub,1)/1.414;
    h = size(I_sub,1)/3;
    for tt = 1:size(p0,1)
        rois{tt} = hingedSample(p0(tt,:),size(I_sub,1)/6,h,thetas);
    end
    rois = cat(2,rois{:});
    rois = cellfun2(@(x) poly2mask2(x.xy,size2(I_sub)), rois);
%     rois = cellfun2(@(x) imdilate(x,ones(7)), rois);
    
%     [res] = directionalROI_rect(startPt,theta,roiWidth,roiLength)
...
%     rois = cellfun2(@(x) poly2mask2(x,size2(I_sub)), rois);
    ...
    patches = cellfun2(@(x) maskedPatch(I_sub,x,true,.5),rois);
    curFeats = featureExtractor.extractFeaturesMulti(patches);
    curScores = w'*curFeats;
    S = computeHeatMap_regions(I_sub,rois,normalise(curScores(:)).^2,'max');
    RR = sc(cat(3,S,im2double(I_sub)),'prob');
    clf
    subplot(1,2,1); imagesc2(RR);
    title(num2str(max(curScores)));
...
for it = 1:1:length(curSet)
    ...
    [mouthBox,candidates,isvalid] = getCandidateRegions(conf,imgData);
    if ~isvalid
        continue
    end
