function follow_geometry_graph(conf,I,lines_,ellipses_,imageSet,imageID,face_poly,mouth_poly)
...
% figure,imagesc(ucm);
img = 1*(1-gpb_thin);
img(gpb_thin<=.1) = 10000;
[D R] = DT(img);
D = (imdilate(gpb_thin,ones(3)));
% figure,imagesc(D)
...
% D_path = shortestPath(D,I,mean(mouth_poly,1));

ellipseFeats = getEllipseFeatures(ellipses_,D);
...
lineFeats = getLineFeatures(I,lines_,D);
...
polygonFeats = getPolygonFeatures({mouth_poly,face_poly},D);
...
%
debug_findPrimitive(primitives,[271.9 98.17]); % line
...
% define a graphical model for the cup
nNodes = 4;
nStates = length(primitives);
% there are two graphs : connectivity between the nodes and between
% the primitives
[G,IG] = findAdjacency(primitives);

G = G.*(G<5);
% calculate pairwise features between primitives.
T = getAllPairwiseFeatures(primitives,G,IG);
...
% unary potentials : first ellipse is special.
types = cellfun(@(x) x.typeID, primitives);
edgeScores =cellfun(@(x) x.energy, primitives);
nodePot(1,findCupEllipses(ellipseFeats,globalData)) = 1;
% nodePot(1,1) = 1;
nodePot(2,:) = double(types == pTypes.TYPE_LINE);
nodePot(3,:) = double(types == pTypes.TYPE_ELLIPSE);
nodePot(4,:) = double(types == pTypes.TYPE_LINE);

% multiply all by edge score.
% nodePot = bsxfun(@times,nodePot,edgeScores);
% nodePot = 10*nodePot;

% binary potentials.
edgePot = zeros(nStates,nStates,edgeStruct.nEdges);
[ii,jj] = find(G);
inds = sub2ind(size(G),ii,jj);
%  1->2 
turnDirections = zeros(size(G));
turnDirections(inds) = cellfun(@(x) x.turnDirection,T(inds));
% z = exp(-abs(turnDirections-90)*.1);
% f = find(z(1,:));
% v = z(1,f);
% 
% [m,im] = sort(v,'descend');
...
%         m(j)
%         plotStuff(primitives{1},'g','LineWidth',2);
%         plotStuff(primitives{f(im(j))},'r','LineWidth',2);
%     pause
% end

edge1 = intersect(UGM_getEdges(1,edgeStruct),UGM_getEdges(2,edgeStruct));
ellipseToLine = zeros(size(G));
ellipseToLine(1:length(ellipseFeats),length(ellipseFeats)+1:end) = 1;
ellipseToLine = max(ellipseToLine,ellipseToLine');
edgePot(:,:,edge1) = exp(-abs(turnDirections-90)*.1); % 1->2
edgePot(:,:,edge1) = edgePot(:,:,edge1).*ellipseToLine;
edge2 = intersect(UGM_getEdges(2,edgeStruct),UGM_getEdges(3,edgeStruct));
edgePot(:,:,edge2) = exp(-abs(turnDirections-90)*.1); % 2->3
edgePot(:,:,edge2) = edgePot(:,:,edge2).*ellipseToLine;
edge3 = intersect(UGM_getEdges(3,edgeStruct),UGM_getEdges(4,edgeStruct));
edgePot(:,:,edge3) = exp(-abs(turnDirections-90)*.1); % 3->4
edgePot(:,:,edge3) = edgePot(:,:,edge3).*ellipseToLine;
edge4 = intersect(UGM_getEdges(4,edgeStruct),UGM_getEdges(1,edgeStruct));
edgePot(:,:,edge4) = exp(-abs(turnDirections-90)*.1); % 4->1
edgePot(:,:,edge4) = edgePot(:,:,edge4).*ellipseToLine;
edge5 = intersect(UGM_getEdges(1,edgeStruct),UGM_getEdges(3,edgeStruct));
edgePot(:,:,edge5) = G > 10;

for k = 1:size(edgePot,3)
    edgePot(:,:,k) = edgePot(:,:,k).*(G>0).*exp(-G);
end

% zero out the edge potentials between incompatible primitive types.
...
colors = {'r','g','b','m'};
decoding=  unique(decoding','rows')';
for j = 1:size(decoding,2)
    clf; imagesc(I); hold on; axis image;
    for k = 1:size(decoding,1)
        plotStuff(primitives{decoding(k,j)},colors{k},'LineWidth',2);
    end
%     pause
end
...
% 
figure,imagesc(I); hold on; axis image;
for k = 1:length(primitives)
    plotStuff(primitives{k},'g');
end

xy = zeros(length(primitives),2);
for k = 1:size(xy,1)
     p1 = primitives{k};
    if (p1.typeID==pTypes.TYPE_LINE)
        xy1 = mean(p1.xy,1);
    else
        xy1 = p1.curveCenter;
    end
...
[m,im] = sort(nodePot(1,:),'descend');
for j = 1:length(m)
    %j
    clf; imagesc(I); hold on; axis image;    
        m(j)
        plotStuff(primitives{im(j)},'g');    
    pause
end

% line_paipowerrs = getParallelLines(primitives,G,IG);
% return;
...
p = primitives(startNodes);
energies = cellfun(@(x) x.energy,p);
[m,im] = sort(energies,'descend');
% for q = 1:length(m)
%     clf;  imagesc(I); axis image;hold on;
%     p_ = primitives{im(q)};
...
% push them not into a stack, but into a priority queue!!
% first try : sort current states by "Goodness"
...
while ~S.state.isempty()
    ...
    ig = IG(currentNode,N);
    % best first search : first, collect all possible next steps.
    % then, sort them
    for iN = 1:length(N)
        curNeighbor = N(iN);
        ...
        currentParams.history = currentVis;
...
E = req;
function E = findCupEllipses(ellipseFeats,globalData)
...
goods = goods & abs(ellipse_n_n*[0 1]') > .7;
...
function feats = getPolygonFeatures(polygons,D)
global pTypes;
...
% apply a rule to a state. A rule may produce multiple state (for
% alternative paths).
global pTypes;
nextState = [];
nextStateParams = [];
...
switch (state)
    ...
    case states.state_bottle_line_1
        ff = f.turnDirection;
        dd = 20;
        if (abs(sind(ff)) < sind(dd))
            %if (abs(ff-180) < dd || ff < dd || (f
            line_T = f.line_T;
            mouthCenter = globalData.mouth_poly.center;
            %
            n1 = normalize_vec(mouthCenter-line_T(1:2),2);
            ...
            if (n2*n1' > .7)
                ...
                if (isContinuationOf(f.line_S,f.line_T))
                    ...
                    if (inBox(inflatebbox(globalData.face_box,[1.3 1.3],'both',false),...
                            f.line_T(1:2)+f.line_T(3:4)))
    ...
    case states.state_ellipse_perp_line
        ...
        elseif (p2.typeID == pTypes.TYPE_ELLIPSE) % look for ellipse at bottom of line...
            ...
            if (r && b && ((abs(f.turnDirection - 90) < 20) || (abs(f.turnDirection - 270) < 20)))
                ...
                areaGrowth = checkAreaGrowth(primitives(stateParams.history),p2);
                if (areaGrowth <= 3)
                    nextState = states.state_accept;
                end
            end
        ...
        if (p2.typeID == pTypes.TYPE_ELLIPSE)
            ...
            if (norm(p1.toFace) < norm(p2.toFace))
                ...
                if abs(f.turnDirection - 270) < 20 || abs(f.turnDirection - 90) < 20
                    ...
                    if (abs(f.turnDirection - stateParams.firstTurn) < 30)
                        
                        areaGrowth = checkAreaGrowth(primitives(stateParams.history),p2);
                        % make sure area is not "exaggerated"
...
function areaGrowth = checkAreaGrowth(historyPrimitives,p2)
xy = [];
for ih = 1:length(historyPrimitives)
    xy = [xy;historyPrimitives{ih}.xy];
end

kk = convhull(xy);
A1 = polyarea(xy(kk,1),xy(kk,2));
xy = [xy;p2.xy];
kk = convhull(xy);
A2 = polyarea(xy(kk,1),xy(kk,2));

areaGrowth = A2/A1;

function T = getAllPairwiseFeatures(primitives,G,G12)
[ii,jj] = find(G);
T = cell(size(G));
for k = 1:length(ii)
    u = ii(k); v = jj(k);
    T{u,v} = getPairwiseFeatures(primitives{u},primitives{v},G12(u,v));
end
