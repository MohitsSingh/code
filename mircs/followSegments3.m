function [parts,allRegions,scores] = followSegments3(conf,regions,G,regionConfs,I,...
    startBox,faceBox,regionOverlaps,binaryModels,binaryScores)
parts = [];
allRegions = [];
scores = [];
%%%%%DEBUGGING FLAG %%%%
...
% straw candidates: long objects intersecting with the lips.

% cup candidates: should be quite outside the face, and not much larger.
% size.
...
% make sure that hand does not intersect face at all (not more than a bit)
% and that cup does intersect face.
% regionConfs(1).score = regionConfs(1).score + 100*ovp_lip';%-50*ovp_face';
regionConfs(1).score(ovp_lip==0)=-inf
regionConfs(2).score = regionConfs(2).score + -100*ovp_face;
...
for iStart = 1:length(startRegions)
    ...
    %regionGroups = regionGroups(1:end);
    ...
    for iOrder = 1:length(modelOrders);
        ...
        % make sure that there is no face overlap for the parts.
        %t = [1 1 1];
...
if (~debug_ && ~showDebug)
    scores = r(1:min(length(r),5));
    allRegions = allRegions(ir(1:min(length(r),5)));
    ...
    while (curCount < 3)
        ...
        modelOrder = modelOrders{allOrders(ir(k))};
        disp(['regions: ' num2str(allRegions{ir(k)})]);
