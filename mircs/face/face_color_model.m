...
z = train_faces(ir_train(train_labels_sorted));
z = z(1:40);
figure,imshow(multiImage(z))
%%
allColors = {};
for k = 1:length(z)
    k
    im = (vl_rgb2xyz((im2double(z{k}))));
%     im = im/max(im(:));
%     
%     im = im(end/4:end/2,end/4:end/2,:);
    
%     imshow(im);
%     pause;
    t = reshape(im,[],size(im,3));
    allColors{k} = t(1:100:end,:);
end

obj = gmdistribution.fit(cat(1,allColors{:}),10);
%%

for k = 1:length(z)
    k
    
    curStuff =  im2double(getImage(conf,train_ids{k}));
    im = (vl_rgb2xyz((curStuff)));
%       im = im/max(im(:));
    t = reshape(im,[],size(im,3));
    h = pdf(obj,t);
    ...
    clf;
    figure(1);
    subplot(1,2,1);imshow(curStuff);
    Z0(isnan(Z0)) = 0;
    
    subplot(1,2,2);imshow(jettify(Z0));
    pause;
...
II = (I(1:end/2,:,:))
landmarks = detect_landmarks(conf,{II},1);
...
matlabpool;
faceLandmarks_train_r = detect_landmarks(conf,train_ids(train_labels),1,true);

[faceLandmarks_train_rr,lipBoxes_train_r,faceBoxes_train_r] = landmarks2struct(faceLandmarks_train_r);

face_imgs_train = multiCrop(conf,train_ids(train_labels),round(faceBoxes_train_r));
