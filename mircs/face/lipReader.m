...
the_q = q_test_cup3;
the_q = arrangeDet(the_q,'index');

newScores = the_q.cluster_locs(:,12) +0*10*double(ismember([faceLandmarks_test_t.c],[5:9])') + .5*test_faces_scores_r';
% multiWrite(lipImages_test(ir),'~/lips_sorted');
% [r,ir] = sort(newScores,'descend');
%m = showSorted(lipImages_test,newScores,150);
showSorted(lipImages_test,newScores,150);
showSorted(test_faces_2,newScores,150);
figure,imshow(rgb2gray(m));

multiWrite(lipImages_train(t_train),'~/lips_sorted_true');
multiWrite(lipImages_train(~t_train),'~/lips_sorted_false');

multiWrite(lipImages_test,'~/lips_test');
...
for k = 1:50
    ...
    skinprob = computeSkinProbability(m);
    figure,imagesc(normalise(skinprob)); axis image; 
    figure,imagesc(m); axis image;


[prec,rec,aps] = calc_aps2(newScores,t_test,sum(test_labels));
[prec,rec,aps] = calc_aps(the_q,t_test,sum(test_labels));
%%
...
% model = trainDPM(conf,r_true,r_false,cls)
...
E = mImage(r);
E = edge(imresize(im2double(rgb2gray(mImage(r))),2,'bilinear'),'canny');
figure,imagesc(E)
