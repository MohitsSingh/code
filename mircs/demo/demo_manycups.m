...
[q_orig_t_d_train,q_orig_t_d_train_det] = applyToSet(conf,q_orig_t_d,train_ids,train_labels,...
    'ids_reorder_train_d12_1','nDetsPerCluster',50,'override',false,'disp_model',true);

f = find(train_labels(q_orig_t_d_train(1).cluster_locs(:,11)));
m = visualizeLocs2(conf,train_ids,q_orig_t_d_train(1).cluster_locs(f(1:50),:));
f = f(1:50);
figure,imshow(multiImage(m))
f([29 33 40 41 44 45 48 49]) = [];

m1 =visualizeLocs2(conf,train_ids,q_orig_t_d_train(1).cluster_locs(f,:));
imshow(multiImage(m1))
...
kk = 1;curSuffix = ['sub_cup' num2str(kk)];
...
conf2.detection.params.detect_add_flip = 1;
T_train = load(fullfile(conf.cachedir,'orig1patches_train.mat'));
[subDetector_trainRes_1,subDet_resDet] = applyToSet(conf2,subDetectors,T_train.a,[],'subDetector_trainRes_1',...
...
Z = cat(2,subDetector_trainRes.cluster_locs);
R = (Z(:,12:12:end));
figure,plot(R)
[m,im] = max(R,[],2)% why so negative??
m(1:5)
im(1:5)
plot(subDetector_trainRes(6).cluster_locs(:,12));
...
%%
Z = createConsistencyMaps(subDetector_trainRes_1,[64 64],[],1,[15 5])
for k = 1:length(Z)
    Z{k}= Z{k} > .5;
end
...
[subDetector_trainRes_1,subDetector_trainRes_1_det] = applyToSet(conf2,subDetectors,T_train.a(1:end),[],'subDetector_trainRes_1',...
    'override',false,'disp_model',true,'dets',subDet_resDet,'useLocation',Z,'uniqueImages',true);

[subDetector_trainRes1] = applyToSet(conf2,subDetectors,....
    T_train.a(train_labels(curDetectorTrain.cluster_locs(:,11))),...
    [],'subDetector_trainRes_n1',...
    'override',false,'disp_model',true,'dets',[],'useLocation',0,'uniqueImages',true,'add_border',true);

figure,plot(subDetector_trainRes1(1).cluster_locs(:,12))
...
[subDetector_testRes1,subDetector_testRes1_det] = applyToSet(conf2,subDetectors,...
    T_test.a(1:end),test_labels(q_orig_t_d_test(1).cluster_locs(:,11)) ,'subDetector_testRes_1',...
    'override',false,'useLocation',Z,'add_border',true,'dets',subDetector_testRes1_det);

% re-train using the true detections vs false detections...
...
% accordingly. 
%%
[M_train,gt_labels_train] = getAttributesForSVM(curDetectorTrain,[subDetector_trainRes_1],train_labels);
sel_ = 1:43;
...
[M_test,gt_labels_test] = getAttributesForSVM(curDetectorTest,subDetector_testRes1,test_labels);
M_test = M_test(:,sel_);
