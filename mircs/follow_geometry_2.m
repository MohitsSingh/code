function follow_geometry_2(conf,I,lines_,ellipses_,imageSet,imageID,face_poly,mouth_poly)
...
if (nargin < 7)
    ...
    [xy_c,mouth_poly,face_poly] = getShiftedLandmarks(imageSet.faceLandmarks(q));%,-imageSet.faceBoxes(q,:));
...
%
debug_findPrimitive(primitives,[147.7 28.45]); % line
...
[G,IG] = findAdjacency2(primitives,ucm);
G = G.*(G <= 30);
...
while ~S.state.isempty()
    ...
    N = find(gg);
    energies = cellfun(@(x) x.energy,primitives(N));
    [m,im] = sort(energies,'descend');
    N = N(im);
    ...
    for iN = 1:length(N)
        ...
        if (nextState==states.state_accept)
            ...
            disp('accepting');
            clf; imagesc(I); axis image; hold on;
            %             plotStuff([primitives{currentVis}],'g','LineWidth',2);
            for k = 1:length(currentVis)
                p_ = primitives{currentVis(k)};
                plotStuff(p_,'g','LineWidth',2);
                %                 plot(p_.xy(:,1),p_.xy(:,2),'G','LineWidth',2);
            end
            %             p_ = primitives{curNeighbor};
            plotStuff(primitives{curNeighbor},'m','LineWidth',2);
            pause;
        end
...
function [G,IG] = findAdjacency2(primitives,ucm)
...
switch (state)
        ...
        if (p2.typeID == pTypes.TYPE_LINE)
            ...
            if (abs(ff- 270)< dd || abs(ff-90) < dd)
                ...
                if (n2*n1' > .7)
                    ...
                    % face.
                    % also, look for a closing (opening) ellipse
    ...
    case states.state_bottle_line_1
        ...
        if (p2.typeID == pTypes.TYPE_LINE)
            ...
            b = b & abs(f.turnDirection - stateParams.firstTurn) < 35;
            ...
            if (b && ((abs(f.turnDirection - 90) < 35) || (abs(f.turnDirection - 270) < 35)))
    ...
    case states.state_straw_to_ellipse
        ...
        if (p2.typeID == pTypes.TYPE_ELLIPSE)
            ...
            % want most ellipse points to be below the straw.
            %ymax = max(stateParams.straw_seg([2 4]));
            
            %y_diff = sum(sign(p2.xy(:,2)-ymax))/length(p2.xy(:,2);
            if (isBelow(p2,p1) > .7)
                %if (y_diff > .7)
...
% check if p2 is "below" p1
function b = isBelow(p2,p1)
b = sum((p2.xy(:,2)-p1.center(2)) > 0)/length(p2.xy(:,2));
function b = midPointDistance(p1,p2)
b = norm(p1.midPoint-p2.midPoint);
