function [M,allLocs,allFeats] = findNeighbors2(conf,samples,ids,suffix...
    ,toSave,do_flip,getQE,addLocation,keepFeats)
...
for k =1:n
    ...
    %     figure,imshow(Z,[]);
    if (getQE)
        E = getQuantizationError(X(1:feat_size,:),conf.dict);
        E = max(E)-E;
        D = bsxfun(@plus,D,E');
    end
...
%         end
        ...
        if (getQE)
            E_flip =  getQuantizationError(X_flip(1:feat_size,:),conf.dict);
            E_flip = max(E_flip)-E;
