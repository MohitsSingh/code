...
for iClass = conf.class_enum.RIDING_A_HORSE
...
% for iClass = 1:length(conf.classes)
    ...
    %vl_pr(2*(test_labels==class_sel)-1,curScores)
...
%         faceBox = T(ir(t)).faceBox;
%         faceBox = inflatebbox(faceBox,4,'both',false);
        %         disp(['orig score: ' num2str(w(1:end-1)'*f.x)]);
        %scoreBefore = r(t);
        ...
        if (blockFace)
            ...
            I1 = I;
            z = box2Region(faceBox,I1);
            z = repmat(z,[1 1 3]);
            I1(~z) = .5;
            f = extractDNNFeats(I1,net_deep,layers,false);
            disp(['score / face: ' num2str(w(1:end-1)'*f.x)]);
        end
...
%         pick = nms(rects,.1);                
        ...
        clf; imagesc2([R])
%         plotBoxes(rects(pick(1:3),:));
        dpc
        
        %I1 = imResample(I,[256 256],'bilinear');
        %faceBox1 = faceBox./fliplr(repmat(size2(I)./size2(I1),1,2));
        %x2(I1); plotBoxes(faceBox1);
        
        %         curFeats1 = extractDNNFeats(I1,net_deep,layers,false);
        %         w(1:end-1)'*curFeats1.x
        %
        %I1(poly2mask2 box2Pts
        if doOcclusion
            ...
            if exist(featsPath,'file')
                ...
                detections = L.rects;
                detections(:,5) = -inf;
                newScores = w(1:end-1)'*cat(2,L.feats{:});
            ...
            imgName =  sprintf('%s_as_%s_(%04.0f).png',curID(1:end-8),conf.classes{iClass},t);            
            imgName1 = strrep(imgName,'_as_','-->');
            imgName1 = strrep(imgName1,'_',' ');
            imgName1 = imgName1(1:end-4);
            imgName1 = strrep(imgName1,'looking','look');
            imgName1 = strrep(imgName1,'writing','write');
            imgName1 = strrep(imgName1,'riding','ride');
            imgName1 = strrep(imgName1,'cleaning','clean');
            imgName1 = strrep(imgName1,'feeding','feed');
            imgName1 = strrep(imgName1,'feeding','feed');
            imgName1 = strrep(imgName1,'phoning','phone');            
            close all
            
            
            [U1] = insertInImage(im2uint8(U), @()text(5,20,imgName1),...
                {'fontweight','bold','color','r','fontsize',10});
            
                    clf; imagesc2(U); dpc;
            %         insertInImage
%             imwrite(U1,fullfile('/home/amirro/notes/images/2015_05_03/',imgName));%,'Mode','lossless');
            continue
        end
        %z = visualizeTerm(detections(:,5),boxCenters(detections),size2(I));
        % now eliminate parts of the image to show the informative bits
        %assert(r(t)==w(1:end-1)'*curFeats.x)
        
        %I = imResample(I,size2(I)*256/size(I,1),'bilinear');
        clf;
        subplot(1,2,1);
        imagesc2(I);plotBoxes(I_rect,'g--');
        plotBoxes(faceBox);
        %         subplot(1,2,2); imagesc2(I1);
        %         subplot(1,2,2); imagesc2(getImage(conf,T(ir(t))))
        dpc;
    end
end
