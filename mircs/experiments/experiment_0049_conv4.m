...
if (~exist('initialized','var'))
    ...
    addpath(genpath('/home/amirro/code/3rdparty/matconvnet-1.0-beta14'));
end
...
%         featureExtractor = DeepFeatureExtractor(conf,true,17); % 16->9216, 17->4096 , 12 (conv4)
        XX = featureExtractor.extractFeaturesMulti(curImgs);
        XX = cat(2,XX{:});
...
%%
knn = 15;
%[iu,dists] = vl_kdtreequery(kdtree,XX,XX(:,1),'NUMNEIGHBORS',knn,'MAXNUMCOMPARISONS',knn*1000);
%%
[iu,dists] = vl_kdtreequery(kdtree,XX,XX(:,1:100),'NUMNEIGHBORS',knn,'MAXNUMCOMPARISONS',knn*100);
showNN(XX,curImgs,knn,false,iu)
%%
showNN(XX(:,1:5:end),curImgs(1:5:end));
feats = XX(:,1:5:end);
kdtree = vl_kdtreebuild(feats);
D = l2(feats',feats');
[u,iu] = sort(D,2,'ascend');

ims = curImgs(1:5:end);
%%
for t = 1:500:length(curImgs)
    [iu_kdtree] = vl_kdtreequery(kdtree,XX,XX(:,t),'NUMNEIGHBORS',knn,'MAXNUMCOMPARISONS',knn*10);
%     iu_l2 = iu(t,1:knn)';
    
    figure(1); clf; imagesc2(curImgs{t});    
    figure(2); clf; 
    subplot(1,2,1); imagesc2(mImage(curImgs(iu_kdtree))); title('kdtree');
%     subplot(1,2,2); imagesc2(mImage(ims(iu_l2))); title('l2');
    dpc
end

%%
% check nearest neighbors vs 
...
%%
kpParams.extractHogsHelper = @(x) {gather(featureExtractor.extractFeatures(x))};
%%
for t = 1:length(fra_db) % 319
    ...
    [I,face_boxes] = getFaceDetectionFRA_DB(conf,fra_db(t));
       figure(3),clf;imagesc2(I); plotBoxes(face_boxes);
    kpParams.debug_ = true;
    bb = round(face_boxes(1,1:4));    
    bb = inflatebbox(bb,1,'both',false);    
%     I = curImgs{t};
%     %I = I+im2uint8(rand(size(I)));
%     bb = [1 1 fliplr(size2(I))];
%         
    %[kp_global] = myFindFacialKeyPoints(conf,I,bb,XX,kdtree,curImgs,ress,ptsData,kpParams);
    [kp_global] = myFindFacialKeyPoints(conf,I,bb,XX,kdtree,curImgs,ress,ptsData,kpParams);
    all_kp_predictions_global(t,:,:) = kp_global;    
    figure(1),clf; imagesc2(I); plotBoxes(kp_global);
    plotBoxes(bb);
    dpc;
