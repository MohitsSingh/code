...
%% the a.p changes
cur_min_score = -inf;
face_score_thresholds = [0:.3:5]
%mixture_coeffs = 0:.1:.5;
mixture_coeffs = .1;
combined_aps = zeros(length(classes),length(face_score_thresholds));
posSamplesRemaining = zeros(length(classes),length(face_score_thresholds));
posSampleRatio = zeros(length(classes),length(face_score_thresholds));
for ii = 1:length(face_score_thresholds)
    ...
    for iClass = 1:4
        ...
        %       displayImageSeries(conf,s40_fra(orig_test),.01)
        
        orig_labels = [s40_fra.classID];
        orig_labels = 2*(orig_labels==curClass)-1;
        posSamplesRemaining(iClass,ii) = sum(orig_labels(orig_test)==1)/sum(orig_labels(~[s40_fra.isTrain])==1);
        posSampleRatio(iClass,ii)= sum(orig_labels(orig_test)==1)/nnz(orig_test);
        orig_scores = -20*ones(size(s40_fra));
        %         orig_scores = orig_scores + rand(size(orig_scores ));
        orig_scores([fra_db(all_test).imgIndex]) = all_face_scores(iClass,:);
        [perf_faces(iClass).recall, perf_faces(iClass).precision, perf_faces(iClass).info] = ...
...
ap_regions = [info_regions.ap]
figure(1);clf;
plot(face_score_thresholds, combined_aps','LineWidth',2); hold on;
%plot([0 0 0 0],ap_mixed','.')
plot(face_score_thresholds, mean(posSamplesRemaining),'k--','LineWidth',2);
plot(face_score_thresholds, mean(posSampleRatio),'m--','LineWidth',2);
legend_str=  conf.classes(classes);
legend_str{end+1} = '% pos sample remaining';
legend_str{end+1} = '# (pos samples)/(total samples)';
ylabel('avg. precision')
xlabel('face thrshold');
legend(legend_str,'interpreter','none')

plot(0,ap_mixed(1),'+','Color',[ 0   0    1.000],'MarkerSize',5,'LineWidth',3);
plot(0,ap_mixed(2),'+','Color',[0 .5 0],'MarkerSize',5,'LineWidth',3);
plot(0,ap_mixed(3),'+','Color',[1 0 0],'MarkerSize',5,'LineWidth',3);
plot(0,ap_mixed(4),'+','Color',[ 0    0.7500    0.7500],'MarkerSize',5,'LineWidth',3);
% figure(2); clf;
