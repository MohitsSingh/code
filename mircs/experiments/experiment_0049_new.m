...
if (~exist('initialized2','var'))
    ...
    if (exist(myDataPath,'file'))
        ...
        [paths,names] = getAllFiles('~/storage/data/ ','.jpg');
        ...
        if (0)
            ...
            for t = 1:length(paths)
                ...
                curBox(1:4) = round(inflatebbox(curBox(1:4),1,'both',false));
            ...
            save ~/storage/misc/aflw_crops.mat ims scores
        end
        %
...
x = rand(100,2)-.5;
p = rand(2,1);
y = x*p; y = y + rand(size(y))*.1;

plot3(x(:,1),x(:,2),y,'r.'); grid on;

my_toy_model = train(y,sparse(x), '-s 11 -B 0');
x_new = 5*(rand(100,2)-.5);
[predicted_label, accuracy, decision_values]=predict(x_new*p,sparse(x_new),my_toy_model);

figure,plot3(x_new(:,1),x_new(:,2),decision_values,'r.'); grid on;
hold on; plot3(x_new(:,1),x_new(:,2),x_new*p,'g.'); 
%hold on; plot(x_new,2*x_new+b,'g.')

%%
% try all at once

% regress the yaw angle
all_yaws
yaw_model = train(all_yaws',sparse(double(feats_fc6)), '-s 11 -B 0','col');
...
%     curFeats = sparse(double(feats_fc6(:,goods)));
    curFeats = sparse(double(XX(:,goods)));
    
    
           
   % fernsStruct(ikpType) = makePointPredictor(feats_fc6(:,goods),cur_kps(goods,:)./repmat(heights(goods)',1,2),1500,2);
    models(ikpType).Bx = train(cur_kps(goods,1)./heights(goods),curFeats, '-s 11 -B 0','col');
    models(ikpType).By = train(cur_kps(goods,2)./heights(goods),curFeats, '-s 11 -B 0','col');
...
% %     
%     curFeats = sparse(double(getImageStackHOG(I,wSize,true,zero_borders )));    
%     curFeats = sparse(double(curFeats));
...
for t = 801:length(fra_db)
    ...
    [~,~,pred_yaw] = predict(0, curFeats,yaw_model,'','col');
