pos_class = 4; % brushing teeth% 6/5/2015
%---------
...
for t = 101:1:length(fra_db)
    I = getImage(conf,fra_db(t));
    clf; imagesc2(I);
    plot_dlib_landmarks(fra_db(t).Landmarks_dlib);
    bb = round(inflatebbox(pts2Box(fra_db(t).Landmarks_dlib),1.5,'both',false));    
        xlim(bb([1 3]));
        ylim(bb([2 4]));
    dpc
end
...
curClassResults = run_pipeline(conf, fra_db(1:1:end), params,posClass, posClassName, featureExtractor);
