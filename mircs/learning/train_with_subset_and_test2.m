...
function [ test_results ] = train_with_subset_and_test2(iClass,features,feature_subset,...
    train_params,labels,valids,train_ids,isTrain,lambdas,toBalance)
...
% train_features_1 = transform_features(features(feature_subset),train_params.features);
debug_range = 1:1:size(train_features_1);
valids_train = valids(isTrain);
valids_train = valids(1:length(train_ids));

[ test_results ] = train_with_subset_and_test(iClass,train_features,feature_subset,...
    train_params,train_labels,test_labels,valids,train_ids,test_features,lambdas,toBalance)

res_train = train_classifiers(train_features_1(debug_range,valids_train),labels(valids_train),train_params,toBalance,lambdas);
