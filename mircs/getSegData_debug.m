function segData = getSegData_debug(conf,imgData,debug_)
if (nargin < 3)
    debug_ = false;
end
mm = 2; nn = 3;
...
segListPath = j2m('~/storage/s40_seg_data_2',imgData);

edgeMap = edge(rgb2gray(M),'canny');

if (exist(segListPath,'file'))
    load(segListPath);
else
    
    p_opts = conf.straw.primitive_opts;
    seglist = getSegmentCandidates(conf,p_opts,M,gPb_thin);               
    save(segListPath,'seglist');
end
...
color_diff = 100*ones(size(pairScores));


% measure the gpb energe inside , but only for edges
% perpendicular to the straw.
%[lineness,symmetry,mag,orientation] = getLineProps(M);
for i1 = 1:size(pairPolys,1)
    for i2 = 1:size(pairPolys,2)
        ...
        if (any(curPoly))
            ...
            bw = poly2mask2(curPoly_reverse,size2(M));
            
            bw_2 = poly2mask2(curPoly_reverse,size2(M));
            bw_1 = poly2mask2(curPoly,size2(M));
            % expect color an appearnce difference....
            [segImage,c] = paintSeg(M,bw_1+2*bw_2);
            colors = meanSegColors(bw_1+2*bw_2,M,false);
            color_diff(i1,i2) = norm(colors(:,1)-colors(:,2));
            
            gpb_reverse_evergy(i1,i2) = mean(mag(bw(:)));
            %             i1 = 77
            %             i2 = 55
            % % %             clf;imagesc2(mag);
...
really_fars = min_mouth_dists > conf.straw.dim/10; % TODO - this was /5,
%%not /10. This causes really_fars_2 to be unneeded
...
color_too_similar = color_diff < 15;
pairScores(color_too_similar) = -100;
pairExplanations = appendString(pairExplanations,color_too_similar,[],'color too similar',debug_);
...
function seglist = getSegmentCandidates(conf,p_opts,M,gPb_thin)
seglist = [];
%     seg_types = {};
for iOpt = 1:length(p_opts)
    curOpt = p_opts(iOpt);
    switch(curOpt.method)
        case 'canny'
            if (~isempty(curOpt.filter))
                M1 = imfilter(rgb2gray(M),curOpt.filter);
                edgeMap = edge(M1,'canny');
            else
                edgeMap = edge(M,'canny');
            end
            [edgelist1] = edgelink(edgeMap, []);
            [seglist_canny,inds] = lineseg(edgelist1,2);
            seglist  =[seglist seglist_canny];
        case 'gpb'
            edgeMap = gPb_thin>.2;
            if (nnz(edgeMap) > 5)
                [edgelist edgeim] = edgelink(edgeMap, []);
                [seglist_gpb,inds] = lineseg(edgelist,2);
                seglist_gpb = cellfun2(@(x) round(x*conf.straw.dim/s1),seglist_gpb);
                seglist  =[seglist seglist_gpb];
            end
    end
end
%
