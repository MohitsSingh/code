...
if conf.tinyProblem
  conf.prefix = 'tiny_tiny';
  conf.numClasses = 5 ;
  conf.numSpatialX = [1 2 4] ;
  conf.numSpatialY = [1 2 4] ;
  conf.numWords = 300 ;
  conf.phowOpts = {'Verbose', 2, 'Sizes', [8], 'Step', 2} ;
end

% h = @(x) 


conf.preprocessfunction = @(x)imresize(x,[32 NaN]);
...
psix = vl_homkermap(hists, 1, 'kinters', 'gamma', .5) ;
...
psix = vl_homkermap(hist, 1, .5, 'kinters') ;
