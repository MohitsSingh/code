function my_ShowBlobs(blobs, numRow, numCol, image, minSize, imNames)
...
for i=1:length(blobs)
    ...
    if blobs{i}.size > minSize
        curBlob = shiftRegions(blobs{i}.mask,blobs{i}.rect([2 1 4 3]),image);
        images{idx} = blendRegion(im2double(image),curBlob);
