...
for i = 1:MAXLEVELS
    ...
    scale(i) = scaler;
    if (scaler > 1)
        scaled = imresize(im,scaler); % TODO - amir, added this for larger than one scales.
    else
        %scaled = resize(im,scale(i));
        scaled = imResample(im,scale(i),'bilinear');
    end
...
%       feat{i} = params.init_params.features(scaled,sbin); % TODO - amir,
    %changed this to vedaldi implementation at Feb. 2 2013 to deal with small
    %images, which have problems at edges with the original descriptor.
    %   feat{i} = double(vl_hog(single(scaled),sbin));
    % todo - amir, changed on Tuesday, 5/11/2013 to fhog by piotr dollar
    feat{i} = double(fhog(scaled,sbin));
    %  feat{i} = vl_hog(scaled,sbin,'NumOrientations',9);
    ...
    %recover lost bin!!!
    %feat{i} = padarray(feat{i}, [1 1 0], 0); % TODO - removed by amir, Feb 5
    %2013, since we switched to vlfeat
