...
for level = length(t.hog):-1:1
    ...
    if params.dfun == 1
        ...
        %Use blas-based fast convolution code
        rootmatch = fconvblas(double(featr), ws, 1, N);
        %% DEBUG 1
        if (debug__)
            disp_ = 0;
            cc= rootmatch{1}-bs{1};
            ...
            if (disp_)
                ...
                plot(sort(cc(:)));
                if (exist('sig_w','var'))
                    %fx = @(x)(1./(1+exp(-beta_(1)*(x-beta_(2)))));
                    hold on;
                    cc_s = sort(cc(:));
                    plot(sigmoid(cc_s*sig_w+sig_b),'r')
                end
                subplot(2,2,3);
                imagesc(I);
...
if (debug__)
    if (~exist('logreg_res.mat','file'))
        ...
        scores =  qqq;
        os = qqq>0;
        [sig_w,sig_b] = logReg(scores,os, invWeights(os));
        %beta_ = esvm_learn_sigmoid(scores, os);
        save('logreg_res.mat','sig_w','sig_b');
    end
end
%% DEBUG 2
