...
% ------------------------------------------------------------------------
function [candidates, ucm2] = im2mcg(image,mode,compute_masks,sf_model)
if nargin<2
    mode = 'fast';
end
if nargin<3
...
if any(empties) % added by amir, 20/10/2014 to avoid crash from empty ms    
    [candidates, ucm2,valid] = im2mcg(image,'fast',compute_masks);    
    return;
end
...
candidates.f_lp = f_lp;
candidates.f_ms = f_ms;
candidates.cand_labels = cand_labels;
% Transform the results to masks
if compute_masks
    candidates.masks = cands2masks(cand_labels, candidates.f_lp, candidates.f_ms);
end
