...
/* Minh Hoai Nguyen's note:
 * This is the function that I copied from Yang & Ramanan's code for for distance transform
 * Note: this function differs from the dt transform function from voc-release for DPM. 
 * The document for this function is limited, contained one line below saying the function is
 * based on Felzenswalb and Huttenlocher.
 *
 * Usage: [adjScore, Ix, Iy] = dt(score, ax, bx, ay, by), compute the adjusted score for spatial deformation:
 *
 * This give: adjScore(x, y) = max_{u,v} score(u,v) - (ax*(x-u)^2 +bx*(x-u) + ay*(y-v)^2 + by*(y-v))
 * This function crashes if ax=bx=0, or if ay=by=0
 * Compilation: 
 * > mex dt_ramanan.cc
*/
