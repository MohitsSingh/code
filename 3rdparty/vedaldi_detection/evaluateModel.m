function [matches, negs] = evaluateModel(...
    testImages, testBoxes, testBoxImages, w, hogCellSize, scales, LL, debugging)
debugging = true;
clear matches ;
negs = {} ;
...
%     findImageIndex(testImages{i})
    
    if (ischar(testImages{i}))
        k = find(cellfun(@any,strfind(LL.imageNames,testImages{i})));
        im = LL.scores_coarse{k};
...
%     im = im2single(im) ;
    ...
    if  (debugging)
        ...
        imagesc(LL.images{k}); ; axis equal ; hold on ;
    ...
    if nargout > 1
        ...
        detections(:, overlaps > 0) = [] ;
        detections = vl_colsubset(detections, 10, 'beginning') ;
        
        
%         imagesc2(LL.images{k});plotBoxes(detections');
