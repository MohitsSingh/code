function landmarks = my_detect_landmarks(conf,images,toScale,useTopHalf,mask)
...
for iImage = 1:length(images),
    ...
    if (nargin >= 3)    
        ...
        mask = imresize(mask,toScale,'nearest');
    end
    
    if (nargin == 4 && useTopHalf)
        im = im(1:floor(end/2),:,:);
        mask = mask(1:floor(end/2),:,:);
    end
            
    %     clf; imagesc(im); axis image; axis off; drawnow;
    
    tic;
    bs = my_detect(im, model, model.thresh,mask);
    bs = clipboxes(im, bs);
    bs = nms_face(bs,0.3);
    im = im2double(im);
    im = min(im,1);
