...
function boxes = detect(input, model, thresh, mask)
...
[pyra,mask_pyra] = my_featpyramid(input,model, double(mask));
...
%             curMask = mask_pyra.feat{level};
            if isempty(resp{level}),
                resp{level} = fconv(pyra.feat{level},filters,1,length(filters));
            end
            
%             for iFilter = 1:length(resp{level})
%                 %resp{level}{iFilter} = resp{level}{iFilter}.*curMask;
%                 resp{level}{iFilter}(~curMask) = 0;%resp{level}{iFilter}.*curMask;
%             end
        ...
        for k = numparts:-1:2,
            ...
            % attenuate part score where mask is 0
            if (any(size(curMask)~=size(msg)))
                curMask = imresize(mask,size(msg),'nearest');
