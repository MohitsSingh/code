function [h,x,y] = plotEllipse2(cRow,cCol,ra,rb,phi,color,nPnts,lw,ls, show)
...
ts = linspace(phi(2),phi(3),nPnts+1);  cts = cos(ts); sts = sin(ts);
x = ra*cts*cos(-phi(1)) + rb*sts*sin(-phi(1)) + cCol;
y = rb*sts*cos(-phi(1)) - ra*cts*sin(-phi(1)) + cRow;
if (show)
    h = plot(x, y);
    set(h,'Color',color,'LineWidth',lw,'LineStyle',ls,'LineSmoothing','off');
%     plot( cCol, cRow, 'k+' );
%     plot( [cCol cCol+cos(-phi(1))*ra], [cRow cRow-sin(-phi(1))*ra] );
%     set([h],'Color',color,'LineWidth',lw,'LineStyle',ls);
else
    h = 0;
end
