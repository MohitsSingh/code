...
%             neededFile = 'brushing_teeth*';

            %neededInds = [96 119 43 178 164 60 129 166 145
            imgFiles=dir([imgDir '/*.jpg']);
            names = {imgFiles.name};
%             A = load('listOfNeededPaths.mat');
%             listOfNeededPaths = A.listOfNeededPaths;
%             goods = ismember(names,listOfNeededPaths);
%             imgFiles=[dir([imgDir '/' neededFile '.jpg'])];
            
            fileNames = cellfun2(@(x) lower(x(1:end-4)),{imgFiles.name});
%             goods = false(size(fileNames));
%             needed = {'blowing','drinking','smoking','phoning'};
%             needed = {'brushing_teeth_179'};
% %             edit findImageIndex.m
%             for iNeeded = 1:length(needed)
%                 a = needed{iNeeded};
%                 goods = goods | cellfun(@any,strfind(fileNames,a));
%             end
%             imgFiles = imgFiles(goods);
