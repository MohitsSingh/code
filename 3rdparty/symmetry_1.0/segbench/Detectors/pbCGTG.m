function [pb,theta,bfeat] = pbCGTG(im,radius,norient)
% function [pb,theta] = pbCGTG(im,radius,norient)
% 
% Compute probability of boundary using CG and TG.
% !!!!MODIFIED VERSION FOR BOUNDARY FEATURE EXTRACTION!!!!
% 
% bfeat: added boundary feature used for training rpb symmetry detector
%
...
if all(radius==[0.01 0.02 0.02 0.02]), % 64 textons
  ...
  error('no parameters for radius=[%g %g]\n',radius(1),radius(2));
end
...
[~,maxo] = max(pball,[],3);
pb = zeros(h,w);
bfeat = zeros(h,w,norient,'single');
theta = zeros(h,w);
r = 2.5;
for i = 1:norient,
  mask = (maxo == i);
  ...
  pb = max(pb,pbi.*mask);
  bfeat(:,:,i) = max(0,a).*mask;
  theta = theta.*~mask + gtheta(i).*mask;
end
pb = max(0,min(1,pb));
bfeat = max(0,min(1,bfeat));
