 ...
 * 
 *
 * warning...not much error checking going on here.  use at your own
 * risk.
 */
...
{
    if (nrhs != 4) mexErrMsgTxt("INPUT:  (lattice.H,lattice.V,radius,sigma) ");
    if (nlhs != 3) mexErrMsgTxt("OUTPUT: [i,j,s] ");

    int dthresh = (int)mxGetScalar(prhs[2]);
    float sigma = (float)mxGetScalar(prhs[3]);
    mexPrintf("Radius = %d\n",dthresh);
    mexPrintf("Sigma = %.3f\n",sigma);
