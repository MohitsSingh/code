function [rec,prec,ap,area_overlap] = VOCevaldet(VOCopts,id,cls,draw)
...
for d=1:nd
    ...
    %%%% change here
    %im = imread(sprintf(VOCopts.imgpath,ids{d}));
    %imc = im(bb(2):bb(4),bb(1):bb(3),:);
    %imshow(imc);
    %disp(sprintf('%f, %d',ovmax, gt(i).det(jmax)));
    %%%% change upto here
    ...
    %keyboard;
    %%%% change upto here
    area_overlap(d) = ovmax;
end

%save('temp.mat','si','tp','area_overlap');
...
if draw
    ...
    xlabel 'recall'
    ylabel 'precision'
 %   title(sprintf('class: %s, subset: %s',cls,VOCopts.testset));
