...
if usechi2,
  tg = zeros(size(tmap));
  for i = 1:ntex,
    % original code: using a logical im cause conv2 to crash on matlab v7
    %im = (tmap==i);
  ...
  tg = 0.5 * tg;
  tg = double(tg);        % ABSURD: if we let tg in single -> overall algo is 5x slower on matlab7 !
else
  [h,w] = size(tmap);
  d = zeros(h*w,ntex);
  for i = 1:ntex,
