...
T_score = 3; % minimal face detection score...
...
curScores = scores(im_subset);
poseMap = [90 -90 30 -30 0 0];
poses_ = poseMap(ress(im_subset,5));
[u,iu] = sort(poses_);
for t = 1:50:length(poses_)
    k = iu(t)
    clf; imagesc2(curImgs{k}); title(num2str(u(t)));
    curScores(k)
    pause
end
...
for iImg =  1:length(fra_db)
    ...
    for u = 1:10
        ...
        if (curImgData.classID ~= conf.class_enum.LOOKING_THROUGH_A_TELESCOPE)
            continue
        end
        ...
        faceBox = inflatebbox(faceBox,[.8],'both',false);
        ...
        %     ff = faceBox;
        I=  flip_image(I);
        I = imrotate(I,20,'bilinear','loose');
