...
switch type
    case 'cofw' % COFW dataset (29 landmarks: X,Y,V)
        nfids=29;D=nfids*3; model.isFace=1;model.name='cofw';
    case 'lfpw' % LFPW dataset (29 landmarks: X,Y)
        nfids=29;D=nfids*2; model.isFace=1;model.name='lfpw';
    case 'helen' % HELEN dataset (194 landmarks: X,Y)
        nfids=194;D=nfids*2; model.isFace=1;model.name='helen';
    case 'lfw' % LFW dataset (10 landmarks: X,Y)
        nfids=10;D=nfids*2; model.isFace=1;model.name='lfw';
    otherwise
        error('unknown type: %s',type);
end
model.nfids=nfids;model.D=D;
end
...
for n=1:M
    ...
    [~,~,Sc,Rot] = translate_scale_rotate(X,Y);
...
function p=initTest(Is,bboxes,model,pStar,pGtN,RT1)
...
elseif(size(bboxes,2)==4 && size(bboxes,3)==RT1)
    ...
    %Previous results are given, use as is (video)
