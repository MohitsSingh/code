...
% add paths
addpath util/
addpath '/home/amirro/code/3rdparty/recognitionUsingRegions/origCode/util';
%%% Parameter Setting
...
for kk =20:100
    close all;
    fn = sprintf('drinking_%03.0f.jpg',kk);
    
    filename = fullfile('/home/amirro/Stanford40_JPEGImages/JPEGImages/',fn);
    L_regions = load(fullfile('~/storage/gpb_s40/',strrep(fn,'.jpg','_regions.mat')));
    L_ucm = load(fullfile('~/storage/gpb_s40/',strrep(fn,'.jpg','_ucm.mat')));
    L_gpb = load(fullfile('~/storage/gpb_s40/',strrep(fn,'.jpg','.mat')));
    % regions = combine_regions_new(L_ucm.ucm,.1);
    regions = L_regions.regions;
    
    
    [hists,rdata] = compute_hists_new(L_gpb.gPb_orient,regions);
...
for rr = 1:length(eregions),
    if eweights(rr) > 0,
        ...
        rectangle('Position',[ebboxes(:,1:2),ebboxes(:,3:4)-ebboxes(:,1:2)],'EdgeColor','r','LineWidth',3);
        title(['weight = ' num2str(eweights(rr))]);
        pause;
    end;
end;
