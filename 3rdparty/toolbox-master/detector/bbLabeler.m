...
usePnts=0; imgApi.setImgDir(imgDir);
    ...
    function api = imgMakeApi()
        ...
        function setImgDir( imgDir1 )
            ...
            %8     'running'
            %9     'takingphoto'
            %10     'usingcomputer'    3,4,5,6,7,9,10,
            % 11    'walking'
            
            
            goods = false(size(imgFiles));
            %cats = {'phoning','playinginstrument','reading','takingphoto','usingcomputer'};
            cats = {'phoning','reading','takingphoto','usingcomputer'};
            for t = 1:length(imgFiles)
                for iCat = 1:length(cats)
                    if any(strfind(imgFiles{t},cats{iCat}))
                        goods(t) = true;
                        break
                    end
                end
...
%             load weirdFiles;
%             for t = 1:length(imgFiles)
%                 for u = 1:length(weirdFiles)
%                     gg = false;
%                     if any(strfind(imgFiles{t},weirdFiles{u}(1:end-1)))
%                         t
%                         gg = true;                        
%                     end
% 
%                     %                 if isempty(strfind(imgFiles{u},'2011_0068'))
%                     %                     goods(u) = false;
%                     %                 end
%                 end
%                 F = goods(t) & gg;
%                 goods(t) = F;
%                 goods(max(t,2)-1) = F;%goods(t) & gg; % get previous file as well.
%             end
...
%              for t = 1:length(imgFiles)
%                  if ~any(strfind(imgFiles{t},'2011_00699')),
%                      goods(t)=false;
%                  end
%              end
