function [imNew] = poissonSolverNeumann(imSrc, imDest, boxSrc, posDest)
%
% rectangular solver
% implementation for Neumann condition
% 
% parameters
% imSrc - source image
...
for y = 1:heightRegion
    for x = 1:widthRegion
        ...
        % 
        % instead of having 4p-q1-q2-q3-q4 = v
        % since we have gradient = (p-q) therefore, for every
        % known gradient, we take away one p and one q
        %
        % for example, if we know g1 = (p-q1) and g2 = (p-q2)
        %
        % we have 2p-q3-q4 = -g1 - g2 + v
        %
        % Notices that the direction of the gradient is really
        % important since our gradient convention is
        % gx = p(x,y) - p(x-1, y) and gy = (p,y) - p(x, y-1)
        %
        %------------------------------------------------------
        countCenterP = 4;
        ...
        if y ~= 1
            M(count, i-L) = -1;
        else % at the top boundary use
            countCenterP = countCenterP-1;
            b(count) = b(count) - (imDest(yd, xd)-imDest(yd-1, xd));
        end
        ...
        if x ~= 1
            ...
            countCenterP = countCenterP-1;            
            b(count) = b(count) - (imDest(yd, xd)-imDest(yd, xd-1));
        end
        ...
        if countCenterP == 0
            fprintf('Error - singularity');
            return;
        end
        
        M(count, i) = countCenterP;
