...
% alpha = 0.2 x (the average gradient norm)
alpha = 0.05 * gradientNorm(imSrc, boxSrc);
beta = 0.6;
...
%             - 4*imSrc(yv, xv));
        ...
        % reduce the guidance vector from
        % alpha^beta * abs(gradient)^-beta * gradient
        
        pq1 = imSrc(yv, xv) - imSrc(yv-1, xv) + episolon;
        pq2 = imSrc(yv, xv) - imSrc(yv+1, xv) + episolon;
        pq3 = imSrc(yv, xv) - imSrc(yv, xv-1) + episolon;
        pq4 = imSrc(yv, xv) - imSrc(yv, xv+1) + episolon;
