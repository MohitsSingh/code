...
{ 
    ...
    DATA = mxCreateNumericMatrix(nDimensions,nSelectRows, mxUINT8_CLASS, mxREAL);
    datap = (unsigned char*) mxGetData(DATA);
	...
	offset = (int64_T) (selectp[i]-1) * (int64_T) (sizeof(unsigned char) * DIMENSIONS);
	...
	fread(&(datap[i*DIMENSIONS]),sizeof(unsigned char),DIMENSIONS,out);
      }
