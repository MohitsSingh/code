...
function model = amir_train_script(cls, n, trainSet, valSet)% Train and evaluate a model. 
%   
...
testset = conf.eval.test_set;% TODO - uncomment later
...
th = tic;
%model = pascal_train(cls, 2, note);
model = my_train(cls, n, note, trainSet);
toc(th);
