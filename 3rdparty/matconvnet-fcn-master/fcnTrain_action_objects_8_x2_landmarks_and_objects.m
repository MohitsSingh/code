% function fcnTrain_action_objects_8_x2_landmarks_and_objects(varargin)
...
opts.expDir = '/net/mraid11/export/data/amirro//fcn/data/fcn8s-action_obj_8_x2_landmarks_and_objects' ;
...
%       dpc
  ...
  for t = 1:length(landmarks)
      newLabels{t} = {masks{t},landmarks{t}};
  end
  imdb.labels = newLabels;
...
% Launch SGD
getBatchWrapper = @(x) @(imdb,batch) getBatch_action_lm(imdb,batch,x,'prefetch',nargout==0) ;
...
% -------------------------------------------------------------------------
% fn = @(imdb,batch) getBatch_action_lm(imdb,batch,opts,'prefetch',nargout==0) ;
