function net = fcnInitializeModel_action_obj_2_class(varargin)
...
net.addLayer('conv_prediction', ...
    dagnn.Conv('size', [12, 12, opts.nClasses+1,opts.nClasses], 'pad', 0),...
    'x38', 'prediction', {'conv_predictionf','conv_predictionb'});

filters = single(randn([12, 12, opts.nClasses+1,opts.nClasses])/100);
f = net.getParamIndex('conv_predictionf') ;
net.params(f).value = filters ;
net.params(f).learningRate = 1 ;
net.params(f).weightDecay = 1 ;

biases = zeros(1,opts.nClasses,'single');
f = net.getParamIndex('conv_predictionb') ;
