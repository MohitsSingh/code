...
opts.expDir = '/net/mraid11/export/data/amirro//fcn/data/fcn8s-fcnTrain_action_objects_classes_subpatches_w_segdata' ;
...
%   train_set = setdiff(1:1215,train_set);
  ...
  for it = 1:1:length(train_val)
      ...
      curPreds = multiCrop2(softmaxScores,boxes);
      imgs{it} = curPatches;
      preds{it} = cellfun2(@im2uint8,curPreds);
...
%   
  ...
  imdb.preds_data = preds;
  imdb.labels = labels;
%   x2(cellfun2(@(x) repmat(single(x)/8,1,1,3),labels))
  
  % generate only training and validation, test will be there later.      
  newIMDBPath = '/net/mraid11/export/data/amirro/fcn/data/fcn8s-fcnTrain_action_objects_classes_subpatches_w_segdata/imdb.mat';
...
stats.rgbMean =[116.6725  111.5917  103.1466]';% ones(1,8)*128]';
...
getBatchWrapper = @(x) @(imdb,batch) getBatch_action_obj(imdb,batch,x,'prefetch',nargout==0,'imageSize',[256 256]) ;
...
test_params    .labels =       {'none','face','hand','drink','smoke','blow','brush','phone'};
test_params.labels_to_block = [];
test_params.prefix = 'perfs_ap';    
test_params.set = 'val';
test = [];    
[perfs,diags] = test_net_perf(opts.expDir,100,imdb,train,val,test,test_params);
