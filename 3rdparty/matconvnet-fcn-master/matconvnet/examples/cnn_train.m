...
if isstr(opts.errorFunction)
  switch opts.errorFunction
    ...
    case 'binary'
     ...
     case 'pdist'
      opts.errorFunction = @error_pdist ;
      if isempty(opts.errorLabels), opts.errorLabels = {'pdist'} ; end
    otherwise
...
for epoch=start+1:opts.numEpochs
  ...
  if ~evaluateMode, 
      ...
      if mod(epoch,opts.save_ratio)==0
        fprintf('saving!\n');
...
function err = error_pdist(opts, labels, res)
    predictions = gather(res(end-1).x);
    
    err = sum( (predictions(:)-labels(:)).^2).^.5;
