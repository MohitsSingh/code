...
opts.expDir = '/net/mraid11/export/data/amirro//fcn/data/fcn8s-non_parametric_shape/theta_scale' ;
...
for t = 1:nImages
    ...
    labels(t) = sub2ind([nRots nScales],iRot,jScale);
...
net = fcnInitializeModel_parametric_prediction_theta('sourceModelPath', opts.sourceModelPath,'nClasses',imdb.nClasses);
...
opts.train.learningRate = 1e-4; % was 1e-4
% Launch SGD
% opts.train = rmfield(opts.train,'trainingOrder');
getBatchWrapper = @(x) @(imdb,batch) getBatch_p_theta(imdb,batch,x,'prefetch',nargout==0) ;
...
modelPath = fullfile(opts.expDir,'net-epoch-30.mat');
net = load(modelPath) ;
...
%     rgb = rgb+abs(.1*randn(size(rgb)));
    ...
    VV = zeros(size(rgb));
    VV(:,:,1) = rgb(:,:,1);
    [curRot,curScale] = ind2sub([nRots nScales],iv);
