...
for i=1:n
    ...
    switch l.type
        ...
        case 'entropyloss'
            res(i+1).x = vl_nnentropyloss(res(i).x, l.class) ;
        case 'softmax'
            res(i+1).x = vl_nnsoftmax(res(i).x) ;
        case 'loss'
            res(i+1).x = vl_nnloss(res(i).x, l.class, [], 'loss',opts.loss) ;
        case 'softmaxloss'
...
if doder
    ...
    for i=n:-1:max(1, n-opts.backPropDepth+1)
        ...
        switch l.type
            ...
            case 'loss'
                res(i).dzdx = vl_nnloss(res(i).x, l.class, res(i+1).dzdx,'loss',opts.loss) ;
            case 'softmaxloss'
                res(i).dzdx = vl_nnsoftmaxloss(res(i).x, l.class, res(i+1).dzdx) ;
            case 'entropyloss'
                res(i).dzdx = vl_nnentropyloss(res(i).x, l.class, res(i+1).dzdx) ;
            case 'relu'
                if isfield(l, 'leak'), leak = {'leak', l.leak} ; else leak = {} ; end
                if ~isempty(res(i).x)
