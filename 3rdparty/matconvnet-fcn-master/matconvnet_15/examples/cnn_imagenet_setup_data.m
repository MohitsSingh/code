...
imdbPath = '~/storage/data/imagenet_2012_imdb.mat';
...
% if exist('train_images_dir.mat','file')
%     load train_images_dir.mat;
% else
%     train_images_dir = dir(fullfile(opts.dataDir, 'images', 'train', 'n*.JPEG'))';
%     N = {train_images_dir.name};
%     N_id = cellfun2(@(x) x(1:9),N);
%     [lia_id,lib_id] = ismember(N_id,cats);
%     save train_images_dir train_images_dir lib_id
% end
...
%
% baseDir = '/home/amirro/storage/data/ILSVRC2012/images/train/';
% for t = 2:length(cats)
%     t
%     curCategory = cats{t};
%     subdir = fullfile(baseDir,curCategory);
%     ensuredir(subdir);
%     fid = fopen('~/tmp_f.txt','w+');
%     cur_files = N(lib_id==t);
%     for p = 1:length(cur_files)
%         fprintf(fid,'%s\n',fullfile(baseDir,cur_files{p}));
%     end
%     fclose(fid)
%     [status,result] = system(sprintf('cat /home/amirro/tmp_f.txt | xargs mv -t %s\n',[subdir '/']));
% end

addpath(genpath('~/code/utils'));

% goods = false(size(names));
...
%     end
% end
% train_images_dir = train_images_dir(goods);
%
% % % cat_count = containers.Map();
% % % for t = 1:length(cats)
% % %     cat_count(cats{t}) = 0;
% % % end
% % %
...
% % %     if N{t}(end-3) == 'J'
% % %        cat_id = N{t}(1:9);
% % %        cat_count(cat_id) =cat_count(cat_id)+1;
% % %     end
% % % end
% % % V = zeros(size(cats));
% % % for t = 1:length(cats)
% % %     V(t) = cat_count(cats{t});
% % % end
% % %
% % % s = textscan(fopen('~/tars.txt'),'%s');s = s{1};
% % %
% % % nFilesInTar = zeros(size(V));
% % % baseDir = '~/storage/data/ILSVRC2012/images/train/';
% % % for u = 1:length(cats)
% % %     u
% % %     tarPath = fullfile(baseDir,[cats{u} '.tar']);
% % %     [status,result] = system(sprintf('tar -tf %s',tarPath));
% % %     result = textscan(result,'%s');
% % %     nFilesInTar(u) = length(result{1});
% % % end
% % %
% % % find(V~=nFilesInTar)
% % %
...
%for i_d = 1:length(train_images_dir)
if (0)
    for i_d = 1:length(cats)
        i_d
        %dir(fullfile(opts.dataDir, 'images', 'train', 'n*'))'
        d = struct('name',cats{i_d});
        %   d = train_images_dir(i_d);
        ...
        if mod(numel(names), 50*100) == 0, fprintf('\n') ; end
...
load('names_and_labels.mat')
%     save names_and_labels.mat names labels
