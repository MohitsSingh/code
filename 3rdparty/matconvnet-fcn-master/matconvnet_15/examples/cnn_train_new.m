...
opts.continue = false ;
opts.skipSaving = false ;
opts.expDir = fullfile('data','exp') ;
opts.conserveMemory = false ;
opts.backPropDepth = +inf ;
opts.printToPdf = true;
opts.sync = false ;
opts.prefetch = false ;
opts.cudnn = true ;
...
opts.loss = 'binaryerror';

%% added by amir, Jan 10, 2016
opts.trainingOrder = 'random';
...
if isstr(opts.errorFunction)
    switch opts.errorFunction
        ...
        case 'attributes'
            opts.errorFunction = @error_multiattributes ;
            if isempty(opts.errorLabels), opts.errorLabels = {'attr1','attr2'} ; end
        otherwise
...
for epoch=start+1:opts.numEpochs
    ...
    switch opts.trainingOrder
        case 'random'            
            train = opts.train(randperm(numel(opts.train))) ; % shuffle
        case 'class_by_class'            
            img_labels = imdb.images.labels(opts.train); % not this will work only for classification...
            [tt,itt] = sort(img_labels);
            train = opts.train(itt);
        case 'fixed'
            train = opts.train;
        case 'adaptive_max_loss'
            train = opts.train;            
            % calc the loss on all training images.
...
function err = error_multiattributes(opts,labels,res)
...
err(2,1) = sum(sum(sum(mass .* min(error(:,:,1:min(size(error,3),5),:),[],3)))) ;
...
% -------------------------------------------------------------------------
function  [net_cpu,stats,prof,per_sample_loss] = process_epoch(opts, getBatch, epoch, subset, learningRate, imdb, net_cpu)
% -------------------------------------------------------------------------
...
% look ahead at K times the needed batch.
% calculate the loss on all samples.
% find 


%% sort the subset by the loss....
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
for t=1:opts.batchSize:numel(subset)
    
    %     if (mod(t,30)==0)
    %     print_toc = toc(print_tic);
    %     if print_toc > 1
    ...
    for s=1:opts.numSubBatches                
        ...
        res = vl_simplenn(net, im, dzdy, res, ...
            ...
            'sync', opts.sync, ...
            'cudnn', opts.cudnn,'loss',opts.loss) ;
        %         T1 = toc(T0)
