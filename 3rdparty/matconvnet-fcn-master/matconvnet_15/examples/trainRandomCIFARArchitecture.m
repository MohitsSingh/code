function net_from_random=trainRandomCIFARArchitecture(K)
opts.expDir = fullfile('data','cifar-lenet');
...
opts.expDir = fullfile('data',['cifar-learn-random-' num2str(K)]);
opts.train.expDir = opts.expDir ;
opts.imdbPath = fullfile(opts.expDir, 'imdb.mat');
...
if exist(opts.imdbPath, 'file')
    ...
    random_nets = {};
    for u = 1:K
        random_nets{u} = cnn_cifar_init_random();
    end
    ...
    batches = batchify(length(sets_new),50);
    all_results = {};
...
opts.K = K;
net_from_random =  cnn_cifar_init_multi(opts);
