function imo = cnn_imagenet_get_batch_and_remember(images, varargin)
...
% if prefetch
%     
%     % %     vl_imreadjpeg(images, 'Prefetch') ;
%     imo = [] ;
%     return ;
% end

% if fetch
im = cellfun(@single,images,'UniformOutput',false);
% end
% if fetch
%     
% %     im = {};
% %     for t = 1:length(images)
% %         im{t} = single(imread(images{t}));
% %     end
%     im = vl_imreadjpeg(images,'numThreads', opts.numThreads) ;
%     
%     for t = 1:length(im)
