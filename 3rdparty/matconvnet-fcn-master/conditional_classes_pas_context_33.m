...
expDir = fullfile(baseDir,'pas_context_33_seg');
ensuredir(expDir);
%
%%
nEpochs = 50;
...
if ~exist(finalModelPath,'file')
    my_fcn_train(imdb,'pas_context_33_seg_full',nEpochs,struct('gpus',1,'freeze',[],'lr_ratio',lr_ratio));
end
% measure network performances at different epochs.

test_params.labels = [{'none',common_labels{:}}];
test_params.labels_to_block = [];
test_params.prefix = 'perfs_frac_10';
test_params.set = 'val';
train = imdb.train;val = imdb.val;test = imdb.test;

test_params.force_check = true;
[perfs,diags] = test_net_perf(expDir,22,imdb,train,val(1:10:end),test,test_params);
%     break
