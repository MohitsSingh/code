function y = getBatch_parts(imdb, images, varargin)
...
si = 1 ;
image_paths = imdb.image_paths;
for i=1:numel(images)

    k = images(i);
    
    rgb = vl_imreadjpeg(image_paths(k));
    rgb = rgb{1};
    load(imdb.labels{k});
    [cls_mask, inst_mask, part_mask] = mat2map(anno, rgb, pimap);
    sel_ = cls_mask == 15;    
    anno = single(sel_).*single(part_mask);
    anno(anno>24)=0;
%     assert(all(anno(:)<=24))        
