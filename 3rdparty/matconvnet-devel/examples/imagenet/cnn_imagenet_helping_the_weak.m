...
% cur_subset = imdb.images.label == 999;
% re-sample the training set so it will contain more weak-class instances.

load weighting;
cur_subset = false(size(imdb.images.label));
for t = 1:length(weighting)
    f = find(imdb.images.label==t);
    cur_jump = round(1/weighting(t));
    f = f(1:cur_jump:end);
    cur_subset(f) = true;
end
% % 
...
imdb.images.label = imdb.images.label(cur_subset);
imdb.images.set = imdb.images.set(cur_subset);
imdb.images.name = imdb.images.name(cur_subset);
imdb.images.id = imdb.images.id(cur_subset);
