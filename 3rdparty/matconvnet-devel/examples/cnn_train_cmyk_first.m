function [net, info] = cnn_train_cmyk_first(net, imdb, getBatch, varargin)
...
for epoch=start+1:opts.numEpochs
    ...
    is_cmyk = imdb.images.is_cmyk(train);
    not_cmyk = ~is_cmyk;
    train = [train(is_cmyk),train(not_cmyk)];
...
%         %       warning('debugging -not prefetching, remove next line later');
%         %       opts.prefetch = false;
...
%         
%         fprintf('debugging - skipping processing to find bad images');
%         disp(t)
% 
%         continue
        if numGpus >= 1
        ...
        %     tic
%         fprintf('%d warning: skipping computation to find bad images!!!!,')
%         continue
