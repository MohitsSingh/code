function h = match_plot_h(img1,img2,points1,points2,d)
%MATCH_PLOT
...
points1 = points1(1:d:end,:);
points2 = points2(1:d:end,:);
%h = figure;
h = clf
...
match_img = zeros(size(img1,2)+size(img2,2),height , size(img2,3),class(img1));
match_img(1:size(img1,1),1:size(img1,2),:) = img1;
match_img(size(img1,2)+1:end,1:size(img2,1),:) = img2;
imshow(im2uint8(match_img));
hold on;
for i=1:size(points1,1)
    plot([points1(i,2) points2(i,2)],...
        [points1(i,1) points2(i,1)+size(img1,2)],colormap{mod(i,6)+1});
end
